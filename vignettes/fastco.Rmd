---
title: "Introducing fastco: Fast Covariance, Correlation, and Cosine Operations"
author: "Drew Schmidt"
date: "`r Sys.Date()`"
output:
  html_document:
...
vignette: >
  %\VignetteIndexEntry{Introducing fastco: Fast Covariance, Correlation, and Cosine Operations}
...
ghit::install_github("wrathematics/fastco")
remotes::install_github("wrathematics/fastco")
```
...
It is additionally simple to verify that these are identical computations:
...
By comparison, the implementation in **fastco** needs no additional storage for computing cosines (to be more precise, the storage is $O(1)$).  The implementation for two dense vector inputs is dominated by the product \code{t(x) \%*\% y} performed by the **BLAS** subroutine  \code{dgemm} and the normalizing products \code{t(y) \%*\% y}, each computed via the **BLAS** function \code{dsyrk}.  For more details, see the vignette *Algorithms and Benchmarks for the fastco Package*.
