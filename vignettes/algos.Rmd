---
title: "Algorithms and Benchmarks for the fastco Package"
author: "Drew Schmidt"
date: "`r Sys.Date()`"
output:
  html_document:
...
vignette: >
  %\VignetteIndexEntry{Algorithms and Benchmarks for the fastco Package}
...
**fastco** [[@fastco]]
**OpenMP** [[@openmp]]
**BLAS** [[@lawson1979basic]]
*Introducing fastco: Fast Covariance, Correlation, and Cosine Operations* [[@fastco2016guide]]
...
The worst case run-time complexity occurs when the matrix is dense but stored as a sparse matrix, and is $O(mn^2)$, the same as in the dense case.  However, this will cause serious cache thrashing, and the performance will be abysmal.
...
m <- 6000
n <- 250
dense <- fastco:::dense_stored_sparse_mat(m, n, .001)
...
```r
benchmark(dense=fastco::cosine(dense), fastco::cosine(sparse), columns=cols, replications=reps)
##     test replications elapsed relative
...
```r
dense <- fastco:::dense_stored_sparse_mat(m, n, .01)
sparse <- slam::as.simple_triplet_matrix(dense)
...
benchmark(fastco::cosine(dense), fastco::cosine(sparse), as.matrix(sparse), columns=cols, replications=reps)
